...
function LoginSignup() {
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <div className=" bg-black w-1/5 h-2/3 rounded-[25px] flex flex-col items-center justify-between px-10 py-5">
        <div className="flex rounded-full bg-white justify-between w-full p-1">
          <p
            onClick={() => toggleOption("login")}
            className={`cursor-pointer rounded-full ${option === "login" ? "bg-black text-white" : "text-black"} px-10 py-2`}
          >
          Login
        </p>
        <p className={`cursor-pointer rounded-full px-5 py-2 ${option === "signup" ? "bg-black text-white" : "text-black"}`} onClick={() => { toggleOption("signup") }}>Signup</p>
      </div>
